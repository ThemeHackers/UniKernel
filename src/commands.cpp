...
void registerCommands() {
    ...
    commandTable.emplace_back("uptime", handle_uptime, false, "Show system uptime");
    ...
    commandTable.emplace_back("date", handle_date, false, "Show current date/time");
    ...
    commandTable.emplace_back("write", handle_write, true, "Digital/Analog write");
    commandTable.emplace_back("read", handle_read, true, "Digital/Analog read");
    commandTable.emplace_back("neofetch", handle_neofetch, false, "Show system info");
    ...
    commandTable.emplace_back("append", handle_append, true, "Append text to file");
    ...
    commandTable.emplace_back("chmod", handle_chmod, true, "Change file permissions");
    ...
    commandTable.emplace_back("deepsleep", handle_deepsleep, true, "Enter deep sleep");
    commandTable.emplace_back("firewall", handle_firewall, true, "Manage IP whitelist");
...
void handle_ls(char *args, bool fromSerial) {
    ...
    if (!fromSerial) {
        ...
        for (int i = 0; i < 16; i++) {
            if ((vfs[i].flags & FLAG_ACTIVE) && strcmp(vfs[i].parentDir, currentPath) == 0) {
        ...
        if (vfs[i].flags & FLAG_ACTIVE) {
            ...
            if (pathMatch) {
                ...
                if (isLong) {
                    ...
                    kprint(F(" "));
                    if (vfs[i].flags & FLAG_ISDIR) kprintColor(CLR_BLU);
                    else kprintColor(CLR_GRN);
                    kprint(vfs[i].name);
                    if (vfs[i].flags & FLAG_ISDIR) kprint(F("/"));
                    kprintColor(CLR_RST);
                    kprintln();
                } else {
                    if (vfs[i].flags & FLAG_ISDIR) kprintColor(CLR_BLU);
        ...
        *redir = '\0';
        ...
        if (strcmp(currentPath, "/dev/") == 0) {
            if (strcmp(filename, "led") == 0) {
                ...
                digitalWrite(LED_BUILTIN, (text[0] == '1' || text[0] == 'H' || text[0] == 'h') ? LOW : HIGH);
...
void handle_help(char *args, bool fromSerial) {
    if (fromSerial) {
        kprintColor(CLR_CYN);
        kprintln(F("UniKernel Shell v2.0 - Commands List"));
        kprintln(F("======================================================================="));
        kprintColor(CLR_RST);

        auto printCategory = [](const __FlashStringHelper* title, const char* cmds[]) {
            kprintColor(CLR_YLW);
            kprintln(title);
            kprintColor(CLR_RST);
            ...
            for (int i = 0; i < count; i += 2) {
                char left[64] = "";
                char right[64] = "";
                snprintf(left, sizeof(left), " %-9s - %-20.20s", foundCmds[i]->name, foundCmds[i]->help);
                if (i + 1 < count) {
                    snprintf(right, sizeof(right), " %-9s - %-20.20s", foundCmds[i+1]->name, foundCmds[i+1]->help);
                    char buf[128];
        ...
        const char* catFs[] = {"ls", "cat", "mkdir", "touch", "cd", "pwd", "echo", "append", "rm", "mv", "cp", "info", "lfs", "chmod", "chown", "df", "save", "load", nullptr};
        const char* catSys[] = {"hwinfo", "uptime", "free", "date", "sys", "neofetch", "cpu", "sleep", "deepsleep", "reboot", "boot", "dmesg", "uname", nullptr};
...
void handle_mv(char *args, bool fromSerial) {
    char *sp = strchr(args, ' ');
    if (!sp) { sendResponse(false, 400, "Usage: mv [src] [dst]"); return; }
    *sp = '\0';
    char *src = args;
    char *dst = kTrim(sp + 1);
...
void handle_cp(char *args, bool fromSerial) {
    char *sp = strchr(args, ' ');
    if (!sp) { sendResponse(false, 400, "Usage: cp [src] [dst]"); return; }
    *sp = '\0';
    char *src = args;
    char *dst = kTrim(sp + 1);
...
void handle_pinmode(char *args, bool fromSerial) {
    char *sp = strchr(args, ' ');
    if (!sp) { sendResponse(false, 400, "Usage: pinmode [pin] [in|out]"); return; }
    *sp = '\0';
    int pin = atoi_safe(args);
...
void handle_write(char *args, bool fromSerial) {
    char *sp = strchr(args, ' ');
    if (!sp) { sendResponse(false, 400, "Usage: write [pin] [0|1]"); return; }
    *sp = '\0';
    int pin = atoi_safe(args);
...
void handle_wifi(char *args, bool fromSerial) {
    ...
    if (strcmp(args, "scan") == 0) {
        if (fromSerial) kprintln(F("Scanning WiFi networks..."));
...
void handle_color(char *args, bool fromSerial) {
    if (strcmp(args, "on") == 0) useColor = true;
    else if (strcmp(args, "off") == 0) useColor = false;
    else {
...
void handle_sys(char *args, bool fromSerial) {
    ...
    if (strcmp(args, "diagnosis") == 0) {
        ...
        kprintColor(CLR_GRN); kprintln(F("[+] Core loop performance is optimal.")); kprintColor(CLR_RST);
        ...
        if (strlen(whitelistIP) == 0) {
            ...
            score -= 20;
        } else kprintln(F("[+] Security: Firewall active."));
        kprint(F("System Security Score: ")); kprintln(score);
    } else if (strcmp(args, "backup") == 0) {
        kprintln(F("--- UniKernel VFS Backup Script ---"));
        for (int i = 0; i < 16; i++) {
            if ((vfs[i].flags & FLAG_ACTIVE) && !(vfs[i].flags & FLAG_ISDIR)) {
                kprint(F("echo \"")); kprint(vfs[i].content); kprint(F("\" > ")); kprintln(vfs[i].name);
            }
        }
    } else {
    ...
    if (fromSerial) {
        ...
        for (int i = 0; i < MAX_TASKS; i++) {
            if (taskTable[i].active) {
                hasTasks = true;
                char buf[64];
                snprintf(buf, sizeof(buf), "%-5d %-14lu %-10lu %s", 
                    i, taskTable[i].interval, taskTable[i].executionCount, taskTable[i].name);
                kprintln(buf);
            }
        }
        if (!hasTasks) {
...
void handle_top(char *args, bool fromSerial) {
    handle_ps(args, fromSerial); 
}

void handle_append(char *args, bool fromSerial) {
    char *sp = strchr(args, ' ');
    if (!sp) { sendResponse(false, 400, "Usage: append [file] [text]"); return; }
    *sp = '\0';
    char *filename = args;
...
void handle_chmod(char *args, bool fromSerial) {
    char *sp = strchr(args, ' ');
    if (!sp) { sendResponse(false, 400, "Usage: chmod [mode] [file]"); return; }
    *sp = '\0';
...
void handle_chown(char *args, bool fromSerial) {
    char *sp = strchr(args, ' ');
    if (!sp) { sendResponse(false, 400, "Usage: chown [owner] [file]"); return; }
    *sp = '\0';
...
void handle_boot(char *args, bool fromSerial) {
    if (strcmp(args, "help") == 0) {
        if (fromSerial) {
            ...
            kprintln(F("  boot <script> - Set custom boot script (e.g. boot main.sh)"));
...
void handle_pwm(char *args, bool fromSerial) {
    char *sp = strchr(args, ' ');
    if (!sp) { sendResponse(false, 400, "Usage: pwm [pin] [0-1023]"); return; }
    *sp = '\0';
    int pin = atoi_safe(args);
    int val = atoi_safe(sp + 1);
    analogWrite(pin, val);
...
void handle_gpio(char *args, bool fromSerial) {
    char *sp = strchr(args, ' ');
    if (!sp) { 
        sendResponse(false, 400, "Usage: gpio [pin] [on|off|toggle] OR gpio toggle [pin]"); 
        return; 
    }
    *sp = '\0';
